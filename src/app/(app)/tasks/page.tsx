...
export default function TasksPage() {
  ...
  const checkAlarms = async () => {
    ...
    for (const task of data || []) {
      ...
      const diff = (due.getTime() - now.getTime()) / 60000 // 分
      if (diff > 29 && diff <= 30) {
        new Notification(`⏰ 30分前: ${task.title}`, {
          body: due.toLocaleString('ja-JP'),
          icon: '/favicon.ico',
        })
      }
      if (diff > 9 && diff <= 10) {
        new Notification(`🔔 10分前: ${task.title}`, {
          body: due.toLocaleString('ja-JP'),
          icon: '/favicon.ico',
        })
      }
    ...
    <div>
      ...
      <form onSubmit={addTask} className="bg-slate-800 rounded-2xl border border-slate-700 p-5 mb-6">
        ...
        <div className="space-y-3">
          <input
            ...
            placeholder="タスク名"
            required
            className="w-full px-4 py-2.5 bg-slate-700 border border-slate-600 rounded-xl text-white placeholder-slate-400 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
          />
          <textarea
            value={description}
            ...
            placeholder="メモ（任意）"
            rows={2}
            className="w-full px-4 py-2.5 bg-slate-700 border border-slate-600 rounded-xl text-white placeholder-slate-400 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none"
          />
          <div className="flex gap-3">
            <input
              ...
              className="flex-1 px-4 py-2.5 bg-slate-700 border border-slate-600 rounded-xl text-white text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
            />
            <button
              type="submit"
      ...
      <div className="space-y-3">
        ...
        {tasks.map(task => (
          <div
            key={task.id}
            className={`bg-slate-800 rounded-2xl border p-4 flex items-start gap-4 ${
              isUrgent(task.due_at) ? 'border-yellow-500/60 bg-yellow-500/5' : 'border-slate-700'
            }`}
          >
            <button
              onClick={() => toggleDone(task.id)}
              className="mt-0.5 w-5 h-5 rounded-full border-2 border-slate-500 hover:border-green-400 flex-shrink-0 transition-colors"
            />
            <div className="flex-1 min-w-0">
              <p className="text-white font-medium text-sm">{task.title}</p>
              {task.description && (
                <p className="text-slate-400 text-xs mt-1 whitespace-pre-wrap">{task.description}</p>
              )}
              {task.due_at && (

...
必ずJSON形式のみで出力（前後に説明不要）:
{
  ...
  "steps": {
    "step1": "【STEP1 受付突破のみ】推奨パターン全てを組み合わせた1本の受付突破トーク。架電者は必ず「株式会社デバイスエージェンシーの米山」固定。プレースホルダー禁止。①IT補助金または人手不足補助金が使える旨②補助金申請から導入まで弊社が全て代行する旨を自然に盛り込む。絶対禁止：「情報が見当たらない」「困っているのではないか」「お困りごとがあるのではないか」など相手を困っていると決めつける表現。「まもなく申請締め切り」「締め切りが迫っております」「お急ぎください」など期限を煽る表現も禁止。補助金は「活用できる制度がある」という事実だけを自然に伝えること。あくまで業界トレンドの情報提供として架電する形にする。「ご連絡いたしました」禁止→「ご連絡させていただきました」を使う。トークの最後は必ず「ご支配人様か、ご担当者様はいらっしゃいますでしょうか？」で締める",
